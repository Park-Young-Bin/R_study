...
library(mlbench)
data('PimaIndiansDiabetes2') # 환자의 당뇨병 여부와 관련한 임상 데이터
str(PimaIndiansDiabetes2)

# 2. 결측치 제거
sum(is.na(PimaIndiansDiabetes2)) # 652개
...
# 5. lasso logistic regression analysis
?cv.glmnet # 교차검증
library(glmnet)
set.seed(123)
...
diabetes.cv$lambda.1se # 일정 수준의 예측 정확도를 보장 + 간명한 모델 생성, 일반화 가능성 높음(과적합 적음)
