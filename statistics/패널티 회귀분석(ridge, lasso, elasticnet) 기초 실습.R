...
# 참고 url: https://www.youtube.com/watch?v=3OEwk2VxZdE&list=WL&index=1&t=1170s
...
train <- createDataPartition(y=Boston$medv, # 자동으로 비율 조절
                    p=0.7, # 훈련 데이터 비율
                    list = F # 행렬 인덱스로 출력, defalt(True: 리스트 인덱스)
                    )
head(train)
...
str(Boston.cv) # 정보 확인
Boston.cv$lambda.min # mse를 최소화 하는 실제 lambda 값
...
summary(diff(resamples(models), metric = "RMSE")) # 유의확률이 모두 1이므로 세 회귀모델 간에 예측성능에 대해 유의한 차이 없음

# 결과
# lasso, elastic: 예측 변수 개수가 한 개 줄었음, 따라서 두 회귀모델은 ridge 모델보다 예측모델의 일반화 관점에서 두 회귀모델을  사용하는 것이 바람직함.
